...
static void prv_fan_err_cb(const GpioAddress *address, void *context) {
  ...
  uint8_t reg1 = 0;
  uint8_t reg2 = 0;
  adt7476a_get_status(s_fan_storage.i2c, ADT_I2C_WRITE_ADDRESS, &reg1, &reg2);
...
StatusCode pd_fan_ctrl_init(FanCtrlSettings *settings, bool is_front_pd) {
  ...
  Adt7476aSettings adt_settings = {
    ...
    .i2c_read_addr = ADT_I2C_READ_ADDRESS,
    .i2c_write_addr = ADT_I2C_WRITE_ADDRESS,
